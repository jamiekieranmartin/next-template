...
export const SignIn: React.FC<SignInProps> = ({ loading, error, signIn }) => {
  return (
    <div className="grid bg-white py-8 px-4 shadow-md sm:rounded-lg sm:px-10">
      <button
        disabled={loading}
        onClick={signIn}
        className={`${
      ...
      {!!error && (
        <small className="text-center text-red-400 p-1">{error}</small>
      )}
    </div>
  );
};
